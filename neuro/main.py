...
async def process_query(query: str) -> str:
    """Функция для обработки запроса из Go"""
    try:
        # ОПРЕДЕЛЯЕМ ДИРЕКТОРИЮ СКРИПТА И ИСПОЛЬЗУЕМ АБСОЛЮТНЫЕ ПУТИ
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        async def demo():
            # ТАКЖЕ ИСПОЛЬЗУЕМ АБСОЛЮТНЫЕ ПУТИ ДЛЯ ДЕМО-РЕЖИМА
            script_dir = os.path.dirname(os.path.abspath(__file__))
            data_file = os.path.join(script_dir, 'benefits_database.json')
            keywords_config_file = os.path.join(script_dir, 'keywords_config.json')
